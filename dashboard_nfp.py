...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            dbname=os.environ.get("DB_NAME", "ETL_NFP"),
...
def create_slicing_charts(df):
    ...
    # Presidents mapping (year ranges are inclusive)
    presidents = [
    ...
    # Find presidents in selected interval
    selected_presidents = []
    for start, end, name in presidents:
        if end >= year_range[0] and start <= year_range[1]:
            selected_presidents.append(f"{name} ({start}-{end})")

    presidents_text = ", ".join(selected_presidents) if selected_presidents else "No president data for selected years."
    st.text_input(
        label=f"Presidents of the USA ({year_range[0]}-{year_range[1]})",
    ...
    # Bar graph: Jobs created year by year in selected interval
    df_yearly_jobs = df_avg_jobs.copy()
    df_yearly_jobs['year'] = df_yearly_jobs['date'].dt.year
    yearly_jobs = df_yearly_jobs.groupby('year')['total_nonfarm'].sum().reset_index()

    fig_bar = px.bar(
        yearly_jobs,
        x='year',
        y='total_nonfarm',
        labels={'year': 'Year', 'total_nonfarm': 'Jobs Created'},
        title=f"Jobs Created Year by Year ({year_range[0]}-{year_range[1]})"
    )
    st.plotly_chart(fig_bar, use_container_width=True)

    # Bar chart: Total jobs created per decade (1940-2020)
    st.subheader("Total Jobs Created by Decade (1940-2020)")

    # Ensure year column exists
    df['year'] = df['date'].dt.year

    # Create decade bins for 10-year intervals from 1940 to 2020 only
    decade_bins = list(range(1940, 2030, 10))  # 1940, 1950, ..., 2020, 2030
    decade_labels = [f"{decade}s" for decade in range(1940, 2030, 10)][:-1]  # 1940s to 2020s

    df['decade'] = pd.cut(df['year'], bins=decade_bins, labels=decade_labels, right=False)

    # Aggregate total jobs created per decade
    decade_jobs = df.groupby('decade')['total_nonfarm'].sum().reset_index()
    decade_jobs = decade_jobs[decade_jobs['decade'].isin(decade_labels)]  # Filter only valid decades

    # Assign a color for each decade
    decade_colors = px.colors.qualitative.Safe
    color_map = {label: decade_colors[i % len(decade_colors)] for i, label in enumerate(decade_labels)}
    ...
    fig_decade_bar = px.bar(
        ...
        labels={'decade': 'Decade', 'total_nonfarm': 'Total Jobs Created'},
        title="Total Jobs Created by Decade (1940-2020)"
    )
    st.plotly_chart(fig_decade_bar, use_container_width=True)
...
def create_dicing_charts(df):
    ...
    # Key events mapping for each quarter and year (example data, expand as needed)
    key_events = {
        'Q1': {
            2020: "COVID-19 pandemic begins, major job losses in March.",
            2021: "Vaccine rollout and economic recovery start.",
            2010: "Post-recession job growth resumes.",
            # Add more years and events as needed
        },
        'Q2': {
            2020: "Lockdowns continue, historic unemployment in April-May.",
            2021: "Stimulus checks boost consumer spending.",
            2015: "Steady job growth, tech sector expansion.",
            # Add more years and events as needed
        },
        'Q3': {
            2020: "Gradual reopening, jobs begin to recover.",
            2012: "Election year, moderate job gains.",
            2022: "Inflation concerns and Fed rate hikes.",
            # Add more years and events as needed
        },
        'Q4': {
            2020: "Holiday season, slow recovery.",
            2016: "Election year, stable employment.",
            2023: "Labor market tightness, wage growth.",
            # Add more years and events as needed
        }
    }

    # Get key events for selected quarter and year range
    events_list = []
    for yr in range(year_range[0], year_range[1] + 1):
        event = key_events.get(quarter, {}).get(yr)
        if event:
            events_list.append(f"<b>{yr}:</b> {event}")

    if events_list:
        events_html = "<br>".join(events_list)
    else:
        events_html = "<b>No key events available for this selection.</b>"
